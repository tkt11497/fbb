    ...
    <v-flex xs12 md12 class="mt-6">
      <v-card color="#9ABDE4" min-height="75vh">
        <v-card flat min-height="70vh" color="#9ABDE4">
          ...
          />
          <v-row class="mx-4 my-1" dense v-for="gamerecord in gamerecordRowset" :key="gamerecord.phase">
            <v-col cols="3">{{gamerecord.phase}}</v-col>
            <v-col cols="3"> 2020/09/14 11:59:05</v-col>
            <v-col cols="3"> {{gamerecord.number}}</v-col>
            <v-col cols="3">
              <v-row no-gutters justify="end" class="mr-3">
                <v-btn @click="$router.push(`/gamerecord/gameDetail/${gamerecord.phase}`)">
...
export default {
  ...
  async asyncData({ params, env }) {
    const gamerecordRowset = await axios.get(
      `${env.ApiUrl}/v1/loginbyphase`
    );
    return {
      gamerecordRowset: gamerecordRowset.data.data,
    };
  },
  methods: {
    searchMembers() {
